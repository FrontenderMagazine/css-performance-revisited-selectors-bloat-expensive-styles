...
[Стивом Саудерсом (Steve Souders)][1] в 2009 году. Его используют как
...
В упомянутых ранее тестах Стива Саудерса используется JavaScript-конструктор
...
быстрым и самым медленным селектором. Среди десктопных браузеров выделяется
 IE9 с наибольшей разницей между ними — 31 мс. В остальных разница составляет
 примерно
...
псевдоэлементу ([тест 6][25]), та же история с устройством под управлением
 ...
 Explorer 9 оказался селектор с частичным совпадением по атрибуту
 ([тест 10][26]).
...
загрузка страницы займёт 110 мс. Тест должен давать в результате разницу в 300%,
...
(помните, мы стремимся к 16 мс и ниже). Для сравнения, для первой тестовой
